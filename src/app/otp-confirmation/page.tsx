...
function OTPConfirmation() {
  ...
  return (
    ...
    >
      <div className="w-full max-w-md bg-white dark:bg-white rounded-lg shadow-lg p-8">
        <h2
        ...
        <p className="mb-4 text-center text-slate-500 dark:text-slate-800 text-sm">
          Ingresa el código OTP que te enviamos por correo electrónico.
          <br />
          <span className="font-semibold">
            No compartas tu código OTP con nadie.
          </span>
        </p>
        <form onSubmit={handleSubmit} className="mb-4">
          <label
            htmlFor="otp"
            className="block mb-2 text-sm font-medium text-gray-700 dark:text-slate-800"
          >
            Código OTP
          </label>
          <input
          ...
          />
          <div className="mb-2 text-sm text-slate-500 dark:text-slate-800">
            El código expira en{" "}
            <span className="font-semibold">{formatTime(expiration)}</span>{" "}
            minutos.
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-2 px-4 text-white font-semibold rounded-md mt-2 transition-colors duration-200 hover:opacity-90 disabled:opacity-50"
        ...
        <div className="flex flex-col items-center gap-2">
          <button
            ...
            className="w-full py-2 px-4 rounded-md mt-2 font-semibold transition-colors duration-200"
            style={{
              backgroundColor:
              ...
              color: "white",
              cursor:
                resendCooldown > 0 || resendLoading ? "not-allowed" : "pointer",
            }}
          >
            {resendCooldown > 0
            ...
            <span className="text-xs text-red-500 font-medium">
              {resendCooldown === RESEND_COOLDOWN_SECONDS ? (
                ...
                <>
                  ...
                  <br />
                  Por favor espera {resendCooldown} segundos antes de intentarlo
                  nuevamente.
                </>
              )}
            </span>
