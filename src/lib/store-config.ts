...
class StoreConfigService {
  ...
  async getStoreConfig(storeId: string): Promise<StoreConfig | null> {
    ...
    const envStoreType = process.env
      .NEXT_PUBLIC_STORE_TYPE as keyof typeof defaultStoreConfigs;
  ...
  async createStore(
    ...
    if (typeof window !== "undefined") {
      localStorage.setItem(
        `store_config_${newStore.storeId}`,
