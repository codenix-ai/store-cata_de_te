    ...
    <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          {selectedCategories.map(category => (
            ...
            >
              ...
              {category.name}
              <button type="button" onClick={() => onChange([])} className="ml-2 text-blue-600 hover:text-blue-800">
                ×
              </button>
            </span>
          ))}
        ...
        >
          <div className="flex items-center justify-between">
            <span className="text-gray-700">
              {selectedCategories.length > 0 ? selectedCategories[0].name : 'Seleccionar categoría'}
            </span>
          ...
          <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg">
            ...
            <div className="max-h-60 overflow-y-auto">
              {filteredCategories.length > 0 ? (
                ...
                <div className="px-3 py-2 text-gray-500 text-sm">No se encontraron categorías</div>
              )}
            </div>
          </div>
        )}
      ...
      {selectedCategories.length === 0 && (
        <p className="text-sm text-gray-500">
          {categories.length === 0 ? 'No hay categorías disponibles' : 'Selecciona una categoría para tu producto'}
        </p>
      )}
    </div>
  );
