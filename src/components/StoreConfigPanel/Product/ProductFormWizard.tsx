...
export function ProductFormWizard({ product, onSave, onCancel, loading = false }: ProductFormWizardProps) {
  ...
  useEffect(() => {
    if (product) {
      ...
      setColors(
        (() => {
          ...
          if (product.variants && product.variants.length > 0) {
            ...
            return mergedColors.filter((color, index, self) => index === self.findIndex(c => c.name === color.name));
          }
          return oldColors;
        })()
      );
      setSizes(
        (() => {
          ...
          if (product.variants && product.variants.length > 0) {
            ...
            return mergedSizes.filter((size, index, self) => index === self.findIndex(s => s.name === size.name));
          }
          return oldSizes;
        })()
      );
      ...
      // Deduplicate by id
      const uniqueCategories = mappedCategories.filter((cat, idx, arr) => arr.findIndex(c => c.id === cat.id) === idx);
  ...
  const { data: productVariantsData, refetch: refetchProductVariants } = useQuery(GET_PRODUCT_VARIANTS, {
    variables: { productId: product?.id || '' },
    skip: !product?.id,
    fetchPolicy: 'cache-and-network',
  });
    ...
    if (productVariantsData && productVariantsData.productVariantsByProduct) {
      ...
      setCustomVariants(
        (allVariants as BackendVariant[])
          .filter((v: BackendVariant) => v.type?.toLowerCase() !== 'color' && v.type?.toLowerCase() !== 'size')
          .map(
  ...
  useEffect(() => {
    if (variantCombinations.length > 0) {
      const calculatedStock = variantCombinations.reduce((total, combination) => total + (combination.stock || 0), 0);
      setFormData(prev => ({
        ...prev,
  ...
  const saveVariantCombinations = async (productId: string, createdVariants?: any[]) => {
    ...
    try {
      ...
      const combinationsToCreate = variantCombinations.filter((c: VariantCombination) => !c.stockId);
      const combinationsToUpdate = variantCombinations.filter((c: VariantCombination) => !!c.stockId);
      ...
      if (combinationsToCreate.length > 0) {
        for (const combination of combinationsToCreate) {
          ...
          if (allVariants.length) {
            variantIds = combination.variants
              .map((variant: any) => {
                ...
                if (matched) {
                  console.log(`✅ Found variant match: ${variant.type}:${variant.name} -> ID: ${matched.id}`);
                } else {
                  console.warn(`❌ No variant match found for: ${variant.type}:${variant.name}`);
                  console.log(
                    'Available variants:',
                    allVariants.map((v: any) => `${v.typeVariant || v.type}:${v.nameVariant || v.name}`)
                  );
                }
                return matched?.id;
              })
          ...
          if (!variantIds.length && allVariantIds.length) {
            ...
            console.warn('[saveVariantCombinations] Skipping combination with no variantIds:', combination);
  ...
  const generateAIDescription = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_REST_API_ENDPOINT}/chatbot/create-product-description`, {
        method: 'POST',
        headers: {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Error al generar descripción con IA';
                ...
                {variantCombinations.length > 0 && (
                  <div className="bg-blue-50 border border-blue-200 p-4 rounded-lg">
                    <div className="flex items-center mb-2">
                      ...
                      <span className="text-sm font-medium text-blue-900">Stock Total: {effectiveStock} unidades</span>
                    </div>
        ...
        return (
          <div className="space-y-8">
            <div className="text-center mb-8">
              ...
              <h3 className="text-xl font-semibold text-gray-900">Imágenes y Variantes</h3>
              <p className="text-gray-600 mt-2">Sube fotos de tu producto y define sus variantes</p>
            </div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="bg-blue-50 p-4 rounded-xl mt-4">
                  <h5 className="text-sm font-medium text-blue-900 mb-2">💡 Consejos para mejores fotos:</h5>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-1 gap-8">
                  ...
                  <div className="mt-8 pt-8 border-t border-gray-200">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-lg font-medium text-gray-900">Combinaciones de Variantes</h3>
                    </div>
                    ...
                    <div className="bg-amber-50 border border-amber-200 p-4 rounded-xl mb-6">
                      <div className="flex items-start">
                        ...
                        <div>
                          <h5 className="text-sm font-medium text-amber-900 mb-2">ℹ️ Cálculo Automático de Stock</h5>
                          <div className="text-sm text-amber-800 space-y-1">
                            <p>
                              • El stock total del producto se calcula automáticamente sumando el stock de todas las
                              variantes
                            </p>
                    ...
                    {loadingCombinations && product?.id ? (
                      <div className="flex items-center justify-center py-8">
                        ...
                        <span className="text-gray-600">Cargando combinaciones existentes...</span>
                      </div>
                    ) : (
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
              ...
              <div className="p-6 space-y-4">
                ...
                {variantCombinations.length > 0 && (
                  <div>
                    <h5 className="font-medium text-gray-900">Stock por Variante:</h5>
                    <div className="mt-2 space-y-1">
                      {variantCombinations.map(combination => (
                        <div key={combination.id} className="flex justify-between items-center text-sm">
                          <span className="text-gray-600">{combination.name}:</span>
                          <span className="font-medium">{combination.stock} unidades</span>
                        </div>
                      ))}
                      <div className="flex justify-between items-center text-sm font-medium border-t pt-1 mt-2">
                        <span>Total:</span>
                        <span>{variantCombinations.reduce((sum, c) => sum + c.stock, 0)} unidades</span>
                      </div>
                    </div>
                  </div>
                )}
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-6">
      <div className="bg-white rounded-xl shadow-lg overflow-hidden">
        {/* Header */}
        <div className="border-b bg-gradient-to-r from-slate-50 to-purple-50 p-4 md:p-6">
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-1 md:mt-2 text-center flex flex-col items-center w-full">
                    ...
                    <p className="text-xs text-gray-400 mt-1 hidden lg:block truncate max-w-20">{step.description}</p>
                  </div>

                  {/* Connection line */}
                  {index < steps.length - 1 && (
                    <div
                      className="hidden md:block absolute h-0.5 bg-gray-200 -z-10"
                      style={{
                        top: '24px',
                        left: 'calc(50% + 24px)',
                        right: 'calc(-50% + 24px)',
