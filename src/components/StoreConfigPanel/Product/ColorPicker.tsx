...
export function ColorPicker({ colors, onChange }: ColorPickerProps) {
  ...
  const updateColor = (index: number, field: 'name' | 'hex', value: string) => {
    const updatedColors = colors.map((color, i) => (i === index ? { ...color, [field]: value } : color));
        ...
        <div className="border rounded-lg p-4 bg-gray-50">
          <div className="flex items-end space-x-3">
            <div className="flex-1">
              <label className="block text-xs font-medium text-gray-700 mb-1">Nombre del Color</label>
              <input
                type="text"
                value={newColor.name}
        ...
        <div className="space-y-2">
          {colors.map((color, index) => (
            <div key={color.id} className="flex items-center space-x-3 p-3 border rounded-lg">
              ...
              <button type="button" onClick={() => removeColor(index)} className="p-1 text-red-500 hover:text-red-700">
                <X className="w-4 h-4" />
              </button>
            </div>
          ))}
